...
const buildMessage = ({ message, answer, gem_name: gemName, is_orchestrator: isOrchestrator, error }) => {
  ...
  const tagText = isOrchestrator ? "Orquestrador" : "GEM Ativo";
  ...
  // Preserva quebras de linha e formatação do markdown
  ...
  container.innerHTML = `
    ...
    <p class="chat-message__question"><strong>Você:</strong> ${message}</p>
    <div class="chat-message__answer">${formattedAnswer}</div>
    ${error ? `<p class="chat-message__error">⚠️ Erro: ${error}</p>` : ""}
  `;
    ...
    if (!loadingMessage) {
      ...
      loadingMessage.innerHTML = `
        <div class="loading-content">
          ...
          <span class="loading-text">O GEM está pensando<span class="loading-dots"></span></span>
        </div>
      `;
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const normalized = {
      message: sanitize(payload.message ?? message),
    ...
    const fallback = {
      ...
      answer: "Não foi possível processar sua solicitação. Verifique se o Ollama está rodando e tente novamente.",
      gem_name: null,
...
resetButton.addEventListener("click", async () => {
  if (!confirm("Tem certeza que deseja reiniciar sua jornada? Seu progresso será mantido em backup.")) {
    return;
  }
